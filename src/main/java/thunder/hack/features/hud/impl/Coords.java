...
public class Coords extends HudElement {
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (HudEditor.hudStyle.is(HudEditor.HudStyle.Blurry)) {
            Render2DEngine.drawRoundedBlur(context, pX, getPosY(), FontRenderers.getModulesRenderer().getStringWidth(coordinates) + 21, 13f, 3, HudEditor.blurColor.getValue().getColorObject());
            ...
            Render2DEngine.renderGradientTexture(context, pX + 2, getPosY() + 1, 10, 10, 0, 0, 512, 512, 512, 512,
                    HudEditor.getColor(270), HudEditor.getColor(0), HudEditor.getColor(180), HudEditor.getColor(90));
            Render2DEngine.endRender();
        }

        FontRenderers.getModulesRenderer().drawString(context, coordinates, pX + 18, getPosY() + 5, HudEditor.getColor(1).getRGB());
